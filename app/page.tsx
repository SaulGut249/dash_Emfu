...
export default function Home() {
	...
	const fetchLatestFrames = async () => {
		...
		if (!error && data) {
			...
			const sortedData = (data as Frame[]).sort(
				(a, b) => new Date(a.ts).getTime() - new Date(b.ts).getTime()
			);

			setFrames((prev) => {
				// Simple deduplication strategy: if the latest frame from fetch is newer than what we have, update.
				// Or simpler: just replace the state if we trust the fetch is always "latest 100".
				// However, to avoid UI jitter, we might want to be careful.
				// For now, replacing the list is safe as long as the order is consistent.
				// We can check if the last frame ID is different to avoid unnecessary re-renders if needed,
				// but React state updates are cheap enough for 100 items.

				// Let's just return the new sorted data.
				return sortedData;
			});
	...
	useEffect(() => {
		...
		const intervalId = setInterval(fetchLatestFrames, 5000);
		...
		const channel = supabase
			...
			.on(
				...
				async (payload) => {
					...
					setFrames((prev) => {
						// Check if frame already exists (from polling)
	...
	const avgLatency =
		frames.length > 0
			? frames.slice(-10).reduce((acc, f) => {
					const l = f.latencies
						? Object.values(f.latencies).reduce((a, b) => a + b, 0)
						: 0;
					return acc + l;
	...
	return (
		<div className="flex flex-col gap-6 p-6">
			...
			<section className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
				...
				<MetricCard
					title="Promedio det./frame"
					value={frames.length ? (totalDet / frames.length).toFixed(2) : 0}
				/>
				<MetricCard
